...
int  cma_optimize(int N, double startp[], double *xll, double *xrl, int lambda, double *inrgstddev, int maxfuncalls, double optp[], int *fevals, double *optv, int *iter) {
	cmaes_t evo; /* an CMA-ES type struct or "object" */
	double *arFunvals, *const*pop, *xfinal;
	int i, j, term = TERM_OTHER;
	int dim, k;
...
//	arFunvals = cmaes_init(&evo, 0, NULL, NULL, 0, 0, "initials.par");
	...
	arFunvals = cmaes_init(&evo, N, startp, inrgstddev, 0, lambda, "non");
...
//	cmaes_ReadSignals(&evo, "signals.par");  /* write header and initial values */
	...
	while(!cmaes_TestForTermination(&evo))
	{
		...
		*/
		...
		for (i = 0; i < lambda; ++i) {
			fitfun(pop[i], (int) dim, &arFunvals[i]);
			(*fevals)++;
		}
